...
(function () {
  ...
  try {
    ...
    if (savedActions) {
      CONFIG.categoryActions = JSON.parse(savedActions);
      ...
      const savedFocus = localStorage.getItem('social_shield_focus_mode');
      if (savedTags || savedFocus !== null) {
        const legacyKeep = savedTags ? JSON.parse(savedTags) : ['motivational', 'meme', 'deepdive', 'wholesome'];
        const isLegacyFocus = savedFocus === 'true';
        CONFIG.categoryActions = {
          motivational: isLegacyFocus && !legacyKeep.includes('motivational') ? 'hide' : 'show',
          meme: isLegacyFocus && !legacyKeep.includes('meme') ? 'hide' : 'show',
          deepdive: isLegacyFocus && !legacyKeep.includes('deepdive') ? 'hide' : 'show',
          wholesome: isLegacyFocus && !legacyKeep.includes('wholesome') ? 'hide' : 'show',
          doom: 'hide',
          fomo: 'hide',
          casual: isLegacyFocus && !legacyKeep.includes('casual') ? 'hide' : 'show',
        };
  ...
  const css = `
    ...
    /* --- On-Demand Gemini 3.5 Flash-Lite Summarizer --- */
    .x-jev-summary-btn {
      display: inline-flex !important;
      align-items: center !important;
      gap: 3.5px !important;
  ...
  pill.title = 'Double-click to configure Gemini API Key for TL;DR summaries';
  pill.addEventListener('dblclick', (e) => {
    e.preventDefault();
    e.stopPropagation();
    const updated = window.prompt(
      'Social Shield: Enter your Google AI Studio (Gemini) API Key (leave blank to clear):',
      CONFIG.geminiApiKey || ''
    );
    if (updated !== null) {
      CONFIG.geminiApiKey = updated.trim();
      try {
        if (typeof GM_setValue !== 'undefined') {
          GM_setValue('social_shield_gemini_api_key', CONFIG.geminiApiKey);
        } else {
          if (CONFIG.geminiApiKey) localStorage.setItem('social_shield_gemini_api_key', CONFIG.geminiApiKey);
          else localStorage.removeItem('social_shield_gemini_api_key');
        }
      } catch (err) {}
      alert(CONFIG.geminiApiKey ? 'Gemini API Key saved.' : 'Gemini API Key cleared.');
    }
  });
  ...
  function applyStateToDOM() {
    ...
    // 4. Synchronize badge visibility
        ...
        summaryBtn.onclick = async (e) => {
          ...
          if (!CONFIG.geminiApiKey) {
            ...
            if (userKey && userKey.trim()) {
              ...
              try {
                if (typeof GM_setValue !== 'undefined') {
                  GM_setValue('social_shield_gemini_api_key', userKey.trim());
                } else {
                  localStorage.setItem('social_shield_gemini_api_key', userKey.trim());
                }
              } catch (err) {}
            } else {
              return;
